...
1. `POST /api/invitations` (`company_admin`/`hr`) `{email, role}` na org ativa → cria
   `invitation` pending com token e expiração; dispara e-mail (envio de e-mail via porta em
   `core`; o provedor concreto é detalhe de infra, não desta spec).
2. `GET /api/invitations/{token}` → dados públicos mínimos pra tela de aceite (nome da
   Empresa, e-mail, papel). 404/410 se inválido/expirado.
3. `POST /api/invitations/{token}/accept` `{name?, password}`:
...
1. `POST /api/partners/signup` `{company_name, document?, admin: {name, email, password}}`
...
2. A associação a uma Empresa é o convênio da spec 03: `company_admin` faz
   `POST /api/agreements {partner_id}`. Alternativamente, uma Empresa pode **convidar** um
...
- `POST /partners/signup` e `accept` respeitam a mesma política de senha do login (spec 02).
...
3. `POST /partners/signup` cria org + admin + sessão numa transação atômica; e-mail repetido
   dá 409 sem criar organização órfã.
