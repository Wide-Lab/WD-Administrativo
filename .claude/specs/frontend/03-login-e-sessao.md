...
- `use-session.ts` — hook TanStack Query sobre `GET /api/auth/me`: expõe
...
- `use-login.ts` / `use-logout.ts` — mutations; no sucesso do login, invalida a query de
  sessão e o contexto (spec 04); no logout, limpa o cache e manda pra `/login`.

## Telas

- **`/login`** — cartão central sobre `bg`, logo, campos e-mail/senha, botão primário
...
redireciona pra `/login?next=<rota>`; enquanto carrega, mostra skeleton, não flasheia
...
1. Login válido seta o cookie (pelo backend) e passa a renderizar área autenticada; inválido
   mostra erro genérico e permanece em `/login`.
2. Recarregar a página mantém a sessão (cookie httpOnly + `GET /auth/me`), sem token no JS.
3. Logout limpa o cache do TanStack Query e volta pra `/login`.
